...
class Files:
    def __init__(self):
        ...
        self.terrain_bits_dir = os.path.join(self.extracts_dir, 'Terrain')
        assert os.path.isdir(self.terrain_bits_dir)

        self.bits_dir = os.path.join(self.files_dir, 'Bits')
        assert os.path.isdir(self.bits_dir)

    def copy_extracts(self, extracts_dir, *path):
        src = os.path.join(extracts_dir, *path)
        dst = os.path.join(self.bits_dir, *path)
    ...
    def cleanup_copied_extracts(self, *path):
        shutil.rmtree(os.path.join(self.bits_dir, *path))
        time.sleep(0.1)  # shutil...

    def cleanup_map(self, map_name):
        self.cleanup_copied_extracts('world', 'maps', map_name)
