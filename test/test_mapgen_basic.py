...
class TestMapgen(unittest.TestCase):
    ...
    def test_3_delete_region(self):
        region: Region = Bits(self.bits_dir).maps[self.map_name].get_region(self.region_name)
        region_dir_path = region.gas_dir.path
        delete_region(self.map_name, self.region_name, self.bits_dir)
        self.assertFalse(os.path.exists(region_dir_path))

    def test_4_delete_map(self):
        m: Map = Bits(self.bits_dir).maps[self.map_name]
